...
public class AIController : MonoBehaviour
{
								...
								if (DistanceLock > dis) {
									if (!Target) {
										...
										if (distance > dis && Random.Range (0, 100) > 80) {
											...
											WeaponSelected = Random.Range (0, flight.WeaponControl.WeaponList.Count);	
										}
									}
								}
							}
    ...
    {
		EventsManager<GameObject>.Invoke(AISpawner.AIDeadEventPrefix + tag, gameObject);
    }
    ...
    bool shootTarget (Vector3 targetPos)
	{
		...
		if (distance <= DistanceAttack) {// is in Range ?
			...
			if (dot >= AttackDirection) {// is in Direction ?
				...
				if (distance < DistanceAttack / 5) {// if too close the target , let's turning back a while.
					turnPosition ();	
				}
